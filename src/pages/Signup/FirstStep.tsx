...
interface Props {
  setSection: (param: 'choice' | 'first_step' | 'second_step') => void
  user: UserData
...
const validationSchema = yup.object<Values>({
  ...
  userName: yup
    ...
    .required('Вкажіть ваш нікнейм')
    .min(2, 'Нікнейм повинен містити принанні 2 символи')
    .max(30, 'Нікнейм повинен містити не більше 30 символів'),
})

const FirstStep: FC<Props> = ({ setSection, user, setUser }) => {
  const { mutateAsync, isLoading, isError } = useMutation(checkUserName)
  ...
  return (
    <div className="px-8 py-12 lg:px-16 sm:rounded shadow bg-white relative h-[520px] w-[450px] flex justify-center">
      ...
      <div>
        ...
        <p className="text-center text-neutral-400 text-sm mb-12">Вкажіть ваше ім'я та нікнейм</p>
