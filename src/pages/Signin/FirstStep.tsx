...
const validationSchema = yup.object<Values>({
  ...
  password: yup
    ...
    .max(100, 'Невірний емейл або пароль'),
})

const FirstStep: FC<Props> = ({ setSection }) => {
  const dispatch = useAppDispatch()

  const navigate = useNavigate()

  const { mutateAsync, isLoading, isError } = useMutation<Values, ResponseBody>(signin)
  ...
  return (
    <div className="px-8 py-12 lg:px-16 sm:rounded shadow bg-white relative h-[520px] w-[450px] flex justify-center">
      ...
      <div>
        ...
        <h1 className="text-2xl text-center font-bold mb-6">Увійти</h1>
        <p className="text-center text-neutral-400 text-sm mb-12">Вкажіть ваш емейл та пароль</p>
        ...
        <p className="text-center text-sm text-neutral-400 transition-colors">
          ...
          .
          <br />
          <Link to="/password-reset/request" className="text-sky-500 hover:text-sky-600">
            Забули пароль?
          </Link>
        </p>
      </div>
