...
interface Props {
  setSection: (param: 'request' | 'finish') => void
}
...
const validationSchema = yup.object<Values>({
  email: yup.string().required('Вкажіть ваш емейл').email('Невірний емейл або пароль'),
})

const Request: FC<Props> = ({ setSection }) => {
  const { mutateAsync, isLoading } = useMutation(resetPasswordRequest)
  ...
  return (
    <div className="px-8 py-12 lg:px-16 sm:rounded shadow bg-white relative h-[490px] w-[450px] flex justify-center">
      <div>
        ...
        <p className="text-center text-neutral-400 text-sm mb-12">
          Вкажіть емейл, на який зареєстровано ваш аккаунт. Ми надішлемо лист з подальшими вказівками для відновлення
          пароля.
        </p>
            ...
            {({ values, touched, errors, handleBlur, handleChange, handleSubmit }) => (
              <form onSubmit={handleSubmit}>
                ...
                <LoadingButton
                  isLoading={isLoading}
                  type="submit"
                  disabled={Boolean((touched.email && errors.email) || !values.email || isLoading)}
                >
                  Відправити лист
                </LoadingButton>
              </form>
